<template>
  <section class="main-form mb-5">
    ...
    <button class="btn btn--pink btn--add" @click.prevent="createBlog()">
      Save
    </button>
    ...
    createBlog() {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          try {
            await this.$store.dispatch("blogs/addBlog", this.form);
            this.$message.success("Blog Created Successfully");
            this.$router.push("/blogs");
          } catch (error) {
            this.$message.error("Blog Creation Failed");
          } finally {
            loading.close();
          }
        }
